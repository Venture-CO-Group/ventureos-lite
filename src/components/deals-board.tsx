...
/**
 * Which deal is open, and the way back (playbook-v5 P20/4).
 *
 * `?deal=` has been generated by the project screen since projects shipped and
 * read by nothing, so opening a project's deal landed on the board with
 * nothing selected. It opens the drawer now.
 */
const DEAL_VIEW = {
  deal: idField("deal"),
...
export function DealsBoard({
  ...
  const [dealView, setDealView] = useViewState(DEAL_VIEW);
  ...
  return (
    <div className="max-w-full">
            ...
            >
              ...
              {list.map((c) => (
                ...
                >
                  ...
                  <span className="mb-2 mt-0.5 flex items-baseline gap-1.5 text-[11.5px] text-muted">
                    {/**
                     * The company is a link now (playbook-v5 P20/4). It was
                     * plain text on a card that already knew the id, which
                     * meant the one thing a person wants next — what else is
                     * going on with this client — was three screens away.
                     */}
                    {c.companyId ? (
                      <Link
                        href={`/leads?company=${c.companyId}&from=${encodeURIComponent(`/deals?deal=${c.id}`)}`}
                        data-testid="deal-open-company"
                    ...
                    <button
                      type="button"
                      data-testid="deal-open-tasks"
                      title={
                        c.openTaskCount > 0
                          ? `${c.openTaskCount} open task${c.openTaskCount === 1 ? "" : "s"}`
                          : "No open tasks — add one"
                      }
                      aria-label={`Tasks on ${c.title}`}
                      onClick={() => setDealView({ deal: c.id })}
                      className="ml-auto flex-none rounded-full border border-line px-1.5 text-[10.5px] tabular-nums hover:border-accent hover:text-accent-ink"
                    >
                      ☑ {c.openTaskCount}
                    </button>
                  </span>
      ...
      {dealView.deal && (
        <EntityDrawer
          kind="deal"
          entityId={dealView.deal}
          from={returnPath(dealView.from)}
          onClose={() => setDealView({ deal: null })}
        />
      )}
